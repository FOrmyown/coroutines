...
public final class CoroutineReader {
    ...
    public CoroutineReader(SerializedState.FrameUpdatePoint[] frameUpdatePoints) {
        this(new DefaultCoroutineDeserializer(), frameUpdatePoints, new SerializedState.FrameInterceptPoint[0]);
    }
    ...
    public CoroutineReader(SerializedState.FrameInterceptPoint[] frameInterceptPoints) {
        this(new DefaultCoroutineDeserializer(), new SerializedState.FrameUpdatePoint[0], frameInterceptPoints);
    }
     ...
     * {@link CoroutineRunner}, use {@link #reconstruct(SerializedState) }.
     ...
     * state is being deserialized for was changed but no {@link SerializedState.FrameUpdatePoint} was provided to this class's constructor to
     * handle the changes)
     */
     ...
     * unrecognized method (e.g. a method that's state is being reconstructed for was changed but no {@link SerializedState.FrameUpdatePoint} was
    ...
    public CoroutineRunner reconstruct(SerializedState state) {
        ...
        SerializedState.VersionedFrame[] versionedFrames = state.getFrames();
        ...
        for (int i = versionedFrames.length - 1; i >= 0; i--) {
            SerializedState.VersionedFrame versionedFrame = versionedFrames[i];
            SerializedState.Frame frame = SerializationUtils.calculateCorrectFrameVersion(null, updatersMap, interceptersMap, versionedFrame);
            ...
            SerializedState.Data variables = frame.getVariables();
            SerializedState.Data operands = frame.getOperands();
        ...
        public SerializedState deserialize(byte[] data) {
            ...
            try {
                ...
                for (int i = 0; i < frames.length; i++) {
                    SerializedState.Frame[] possibleFrames = frames[i].getFrames();
                    for (int j = 0; j < possibleFrames.length; j++) {
                        SerializedState.Frame frame = possibleFrames[j];
