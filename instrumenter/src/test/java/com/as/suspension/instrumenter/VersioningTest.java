...
import com.as.suspension.user.SerializedState.FrameInterceptPoint;
...
import static com.as.suspension.user.SerializedState.FrameModifier.WRITE;
import static com.as.suspension.user.SerializedState.FrameModifier.READ;
import com.as.suspension.user.SerializedState.FrameUpdatePoint;
...
public final class VersioningTest {
    ...
    public void mustInterceptOnRead() throws Exception {
        runWrapped(SharedConstants.INTERCEPT_TEST, (classLoader) -> {
            Class<Suspendable> cls = (Class<Suspendable>) classLoader.loadClass(SharedConstants.INTERCEPT_TEST);
            ...
            CoroutineReader reader = new CoroutineReader(
                    new FrameInterceptPoint[]{
                        new FrameInterceptPoint(SharedConstants.INTERCEPT_TEST, -926730864, 0, (frame, mode) -> {
    ...
    public void mustProperlyVersionUp() throws Exception {
        ...
        runWrapped(SharedConstants.UPDATE_TEST_ORIGINAL, (classLoader) -> {
            Class<Suspendable> cls = (Class<Suspendable>) classLoader.loadClass(SharedConstants.UPDATE_TEST);
        ...
        // frame state)
        runWrapped(SharedConstants.UPDATE_TEST_MODIFIED, (classLoader) -> {
            Class<Suspendable> cls = (Class<Suspendable>) classLoader.loadClass(SharedConstants.UPDATE_TEST);

            FrameUpdatePoint updateEchoPoint = new FrameUpdatePoint(SharedConstants.UPDATE_TEST, -526669244, 0, (frame, mode) -> {
                Object[] varObjects =  frame.getVariables().getObjects();
    ...
    public void mustProperlyVersionDown() throws Exception {
        ...
        runWrapped(SharedConstants.UPDATE_TEST_MODIFIED, (classLoader) -> {
            ...
            FrameUpdatePoint downgradeEchoPoint = new FrameUpdatePoint(SharedConstants.UPDATE_TEST, -1238526627, 0, (frame, mode) -> {
                Object[] varObjects =  frame.getVariables().getObjects();
    ...
    private void runWrapped(String name, WrappedTest test) throws Exception {
        ...
        try (URLClassLoader classLoader = TestUtils.loadClassesInZipResourceAndInstrument(name + ".zip", settings)) {
